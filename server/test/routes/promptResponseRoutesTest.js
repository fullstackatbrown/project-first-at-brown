...
describe('Room Routes', () => {
  ...
  describe('POST /prompt/:roomId', () => {
    it('creates a new prompt response', async () => {
      ...
      await makeRoom(jwt, "What's your favorite idea?", new Date('2021-01-01'))
        .expect(200);

      const res = await request.post('/prompt/1')
        .set('Authorization', 'Bearer ' + jwt)
        .send({
          body: "Ice cream!"
        })
  ...
  describe('PUT /prompt/:roomId', () => {
    it('updates a prompt response', async () => {
      ...
      const res = await request.put('/prompt/1')
        .set('Authorization', 'Bearer ' + jwt)
        .send({
          body: "Pizza!"
        })
  ...
  describe('DELETE /prompt/:roomId', () => {
    it('deletes a prompt response', async () => {
      ...
      const res = await request.delete('/prompt/1')
        .set('Authorization', 'Bearer ' + jwt)
        .send()
        .expect(200);
  ...
  describe('POST /prompt/:roomId/report/:accountId', () => {
    it('reports a prompt response', async () => {
      ...
      const res = await request.post('/prompt/1/report/1')
        .set('Authorization', 'Bearer ' + jwt)
        .send()
        .expect(200);
