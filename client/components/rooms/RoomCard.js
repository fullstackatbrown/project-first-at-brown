...
const RoomCard = ({ roomData, navigation }) => {
	...
	const displayNumResponses = () => {
		const pluralizedText = roomData.num_responses === "1" ? " response" : " responses";
	...
	return (
		...
		<View style={styles.card}>
			<Text h4 style={styles.cardHeader}>{roomData.prompt}</Text>
			<View style={styles.cardFooter}>
				<Text style={styles.numResponses}>{displayNumResponses()}</Text>
				<Text style={styles.expiresAt}>closes {renderDate(roomData.expires_at)}</Text>
			</View>
		</View>
		</Pressable>
	);
