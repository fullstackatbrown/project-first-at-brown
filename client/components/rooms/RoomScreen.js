...
const PromptResponse = ({ response, children }) => {
	...
	return <View style={styles.response}>
		<View style={{ flexDirection: "row", marginBottom: 8 }}>
			...
			/>
			<View>
				<Text>{response.first_name} {response.last_name}</Text>
...
const RoomScreen = ({ route }) => {
	...
	const createPromptResponse = async () => {
		...
		if (!response.data.createSuccess) {
			console.log("TODO: something went wrong (i.e. user already made a response)")
		}
		fetchRoom();
	}
	...
	const reportPromptResponse = async (authorId) => {
		const response = await API.post("/prompt/" + roomId + "/report/" + authorId);
		if (response.data.reportSuccess) {
			console.log("TODO: report success message");
		} else {
			console.log("TODO: report failure message (i.e. user already reported this)")
		}
	}
	...
	const renderUserResponse = () => {
		if (userResponse === null) {
			...
			return <View style={styles.responseInput}>
			...
			return <View style={styles.userResponse}>
	...
	const renderResponses = () => {
		return responses.map(r => <PromptResponse key={r.author_id} response={r} />)
	}
